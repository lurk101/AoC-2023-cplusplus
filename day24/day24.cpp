...
static int part1() {
    constexpr double min_area = 200000000000000;
    constexpr double max_area = 400000000000000;
    int sum = 0;
    for (int i = 0; i < stones.size() - 1; i++) {
        for (int j = i + 1; j < stones.size(); j++) {
            ...
            if (det == 0)
                continue;
            double u = (sj.vel.x * (si.pos.y - sj.pos.y) + sj.vel.y * (sj.pos.x - si.pos.x)) / det,
                   t = (si.vel.x * (si.pos.y - sj.pos.y) + si.vel.y * (sj.pos.x - si.pos.x)) / det;
            if (u < 0 || t < 0)
                continue;
            double x = sj.pos.x + sj.vel.x * t, y = sj.pos.y + sj.vel.y * t;
...
static int64_t part2() {
    ...
    while (stones[stone0_ix].vel == stones[stone2_ix].vel ||
        ...
        stone2_ix++;
    double vx0 = stones[stone0_ix].vel.x, vy0 = stones[stone0_ix].vel.y,
    ...
    double abvx = vx0 - vx1, abvy = vy0 - vy1, abvz = vz0 - vz1, acvx = vx0 - vx2, acvy = vy0 - vy2,
           acvz = vz0 - vz2;
    double x0 = stones[stone0_ix].pos.x, x1 = stones[stone1_ix].pos.x, x2 = stones[stone2_ix].pos.x,
           y0 = stones[stone0_ix].pos.y, y1 = stones[stone1_ix].pos.y, y2 = stones[stone2_ix].pos.y,
           z0 = stones[stone0_ix].pos.z, z1 = stones[stone1_ix].pos.z, z2 = stones[stone2_ix].pos.z;
    double abx = x0 - x1, aby = y0 - y1, abz = z0 - z1, acx = x0 - x2, acy = y0 - y2, acz = z0 - z2;
    ...
    double pxx = acvx * abz - abvx * acz, pyy = acvy * abx - abvy * acx,
           pzz = acvz * aby - abvz * acy, pxz = abvx * acx - acvx * abx,
           pzy = abvz * acz - acvz * abz, pyx = abvy * acy - acvy * aby;
    double pxc = abvx * h3 - acvx * h2, pyc = abvy * h1 - acvy * h0, pzc = abvz * h5 - acvz * h4;
    double pxd = acvx * abvz - abvx * acvz, pyd = acvy * abvx - abvy * acvx,
           pzd = acvz * abvy - abvz * acvy;
    double qz0 = (abvy / pxd) * pxz, qx0 = (abvy / pxd) * pxx - (abvx / pyd) * pyx - aby,
           qy0 = abx - (abvx / pyd) * pyy;
    double r0 = h0 - (abvy / pxd) * pxc + (abvx / pyd) * pyc;
    double qy1 = (abvx / pzd) * pzy, qz1 = (abvx / pzd) * pzz - (abvz / pxd) * pxz - abx,
           qx1 = abz - (abvz / pxd) * pxx;
    double r1 = h2 - (abvx / pzd) * pzc + (abvz / pxd) * pxc;
    double qx2 = (abvz / pyd) * pyx, qy2 = (abvz / pyd) * pyy - (abvy / pzd) * pzy - abz,
    ...
    double px = (pxz * qz + pxx * qx + pxc) / pxd, py = (pyx * qx + pyy * qy + pyc) / pyd,
